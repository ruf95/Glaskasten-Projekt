...
CURRENT_COVER_HEIGHT, CURRENT_COVER_WIDTH = (
    810,
    546,
)  # Anzeigegröße der Spielcover bei Auswahl
BG_COVER_HEIGHT, BG_COVER_WIDTH = (
    656,
    441,
)  # Anzeigegröße der Spielcover im Hintergrund
...
class GameEntry:
    def __init__(
        self, title, current_cover_surface, bg_cover_surface, game_path
    ):
        self.title = title
...
def game_loader(COVER_PATH, GAME_PATH):
    ...
    games = {
        f.stem: [f, os.path.join(GAME_PATH, f.stem, f.stem + ".py")]
        for f in COVER_PATH.iterdir()
    ...
    for name in games:
        # Lädt das Cover
        cover_surface = pygame.image.load(str(games[name][0]))
        current_cover_surface = (
            # Skaliert das Cover
...
class picking_wheel:
    ...
    def launch_game(self):
        ...
        try:
            ...
            # Das Hauptskript (das Auswahlrad) wartet hier, bis das Spiel
            # beendet ist.
            subprocess.run(
                [sys.executable, current_game_path],
                check=True,
                # Setzt das Arbeitsverzeichnis für das Spiel
                cwd=os.path.dirname(current_game_path),
            )
            print(f"Spiel beendet. Zurück im Auswahlrad.")

        except subprocess.CalledProcessError as e:
            print(
                f"Fehler beim Starten des Spiels ',{current_game.title}': {e}"
            )
        except FileNotFoundError:
            print("Fehler: Das Spiel-Skript konnte nicht gefunden werden.")

    def update(self):
        # Wenn die Animation läuft bewege current_scrol_pos näher an
        # animation_target
        if self.is_animating:
            diff = (
                self.animation_target - self.current_scrol_pos
            )  # Differenz zwischen Ziel und aktueller Pos
            self.current_scrol_pos += (
                diff * ANIMATIONSPEED
            )  # Zwischenschrittberechnung

            if (
                abs(diff) < 0.01
            ):  # Wenn aktuelle Pos nah genug an Ziel setze auf Ziel
    ...
    def draw(self, screen):
        ...
        if len_games == 0:
            return  # Sicherheitscheck falls keine Spiele da sind
        ...
        for i in range_of_covers:
            ...
            x_pos = (
                COVER_POS_X + (i * SPACING) - global_x_offset
            )  # Verschiebt geladene Cover
...
def main():
    ...
    # Automatische Instanzierung der picking_wheel Klasse
    picker = picking_wheel(initialized_games)
